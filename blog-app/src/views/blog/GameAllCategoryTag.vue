    ...
    <el-container class="me-allct-container">
      <el-main>
        <el-tabs v-model="activeName">
          <el-tab-pane label="Game Category" name="category">
          ...
          <el-tab-pane label="label" name="tag">
            <ul class="me-allct-items">
...
<script>
  import {getAllGameCategorysDetail} from '@/api/gamecategory'
  import {getAllGameTagsDetail} from '@/api/gametag'

  export default {
    name: 'GameAllCategoryTag',
    created() {
      this.getCategorys()
      this.getTags()
    },
    ...
    computed: {
      ...
      categoryTagTitle (){
        ...
        console.info('dddddddddddddddddddddddddddddddddddddddddddsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
        return 'game tag'
      }
    },
    methods: {
      view(id) {
        this.$router.push({path: `/game/${this.currentActiveName}/${id}`})
      },
      getCategorys() {
        let that = this
        getAllGameCategorysDetail().then(data => {
      ...
      getTags() {
        let that = this
        getAllGameTagsDetail().then(data => {
          that.tags = data.data
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: 'load fail', showClose: true})
